...
public class SequenceList<T> implements List<T>{
	...
	private int size = 0;//顺序表的当前大小 count
	...
	public int add(int index,T element){
		if(index<0 || index>this.size){//注意是this.size 没有-1 包含插在了最后
			throw new IndexOutOfBoundsException("线性表索引越界");
		}
		while(this.capacity < (this.size+1)){//若增加的元素使线性表容量不够，则扩容
			this.capacity <<= 1;
			list = Arrays.copyOf(list, this.capacity);
		}
		System.arraycopy(list, index, list, index+1, this.size-index);
		list[index] = element;
		this.size++;
		return 1;
	}
	...
	public String toString() {
		return "List [defautSize=" + defautSize + ", capacity=" + capacity + ", size=" + size + ", list="
